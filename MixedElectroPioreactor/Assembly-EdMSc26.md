...
ed04 is the unit that gets a bagged Pt-plated Ti anode and stainless steel cathode which you can unwrap and install once you are comfortable using ed05. ed05 has all the newest other parts; its electrodes are already installed from bench testing.
...
4. Connect the black 12 V PSU to the Pioreactor HAT's barrel jack connector and the white 5V Raspberry Pi PSU to the micro-USB socket on the Raspberry Pi (the frontmost socket on the bottom PCB if the Pioreactor Logo is facing you). The electroPioreactor should boot within ~90 s.
5. From a laptop connected to the mobile phone hotspot, open `http://ed04.local/` (or `http://ed05.local/`). Confirm the unit appears in the cluster view.
...
**PWM channel assignment for these units:**
PWM 2 = waste pump, PWM 3 = media pump. If you read [Pioreactor's pump documentation](https://docs.pioreactor.com/user-guide/using-pumps) and see PWM 2 used as a media-pump example, ignore that — for ed04/ed05 stick with the canonical 2=waste, 3=media.
...
8. Unwrap the bagged Pt-plated Ti anode. Try to avoid touching the platinised section.
9. Very very gently rotate and push the anode into its o-ring in the Vial Cap.
10. Rotate and adjust the heights of both electrodes so their bases are 33mm below the bottom of the vial cap.
11. Ensure the wires are correctly located in the TopStop so they will come into firm contact with each electrode when the TopStop screws are tightened onto the electrode
12. Push the TopStop down fully onto the electrodes.
13. Tighten the **red** electrode cable onto the Pt-plated Ti **anode** using the M3 bolt through the captive nut.
14. Tighten the **black** electrode cable onto the stainless-steel **cathode** using the M3 bolt through the captive nut.
   - Polarity matters. Reversing red and black dissolves the stainless steel into solution.
15. Reconnect the tubing and screw the vial back into place
