...
4. Eject the card. One change to the rest of Pioreactor's guide: for a unit that makes its own Wi-Fi, join the network `pioreactor` (password `raspberry`) and open `http://pioreactor.local`. Then continue the guide from its step 18; the model dialog at its step 21 does not appear, because the bundle has set the model. **electroPioreactor** is under **Activities** on the unit's *Manage* page, and the precision temperature plugin is installed.
   ...
   \* Wi-Fi rules differ by country, so the file names yours. If you are in the United Kingdom (Great Britain and Northern Ireland), do nothing: it already says `GB`. Otherwise open `local_access_point` in a text editor (TextEdit on a Mac, Notepad on Windows), replace `GB` with your country's two-letter code, and save; the file must contain those two letters and nothing else. The code is the ISO 3166 two-letter one, for example `IE` for Ireland, `DE` for Germany, `US` for the United States: [full list](https://en.wikipedia.org/wiki/ISO_3166-1_alpha-2#Officially_assigned_code_elements).
