...
**Protocol document control.** surface-kla, faradaic-efficiency, knallgas-stoichiometry advanced authored → reviewed (method + cell routing verified, no fixes). dissolved-oxygen, flow-calibration, gerrit-current advanced authored → checked. `vial-geometry.md` and `sinter-porosity.md` held at authored — both carry high-severity routing defects (vial-geometry writes into superseded Vtot_1/2 & D_int_1/2 instead of the reactor-type lookup table A84:I87; sinter-porosity writes into the import cell por_grade / a non-existent "Summary D42 dropdown" instead of the electrode table col E). `authorised:` left empty throughout — that is a human act.
