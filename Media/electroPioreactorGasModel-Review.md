...
**Free-chlorine / bleaching model — under rebuild (the FE=1 ceiling is unphysical).** The earlier `bleach_flag`/`HOCl_max` treatment assumed every anode electron makes free chlorine (faradaic efficiency = 1) and that the whole chloride inventory persists as HOCl, giving ~8.9 mg/L for UdG. That is not physical: at the medium's trace chloride (~0.1–0.2 mM) chlorine evolution is mass-transport-limited to a small fraction of the current and competes with oxygen evolution, and the medium's ammonium exerts a chlorine demand (chloramine formation) that suppresses free HOCl — which is why UdG-type media run for many days to hundreds of days under electrolysis in the published literature. The model is being rebuilt on a realistic chlorine-evolution efficiency minus medium chlorine demand, with a literature-grounded HOB free-chlorine **impairment-onset** threshold (below the WHO kill-all residual, not above it), so the warning fires only on genuinely adverse inputs and the schedule can be dialled until free chlorine is acceptable. The prior 0.2 mg/L threshold and any pH survive/kill framing are **superseded**.
