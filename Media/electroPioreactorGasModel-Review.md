...
**HOCl threshold (H-5).** `bleach_flag` (D109) re-keyed from `HOCl_max > 1` to `HOCl_max > 0.2` mg/L free chlorine — the conservative cell-sensitive first-impairment floor for suspended gram-negative environmental bacteria (necator, Hydrogenophaga, Xanthobacter). A per-genus HOCl-tolerance **Comments column (column G, rows 106–110)** was added recording the evidence: *C. necator* has no direct MIC and its genus-mate *C. metallidurans* is chlorine-TOLERANT; *Hydrogenophaga* is a dominant chlorine-RESISTANT genus; *Xanthobacter* has no published value (genuine gap, not invented). The 0.2 mg/L seed is therefore a deliberate safety FLOOR, very likely an underestimate of the true breaking point for these resistant genera — the flag reads RISK at the current 8.88 mg/L HOCl_max by ~44×.

**Threshold re-cited (precautionary floor, not a MIC).** The 0.2 mg/L free-chlorine figure is the **WHO minimum free-chlorine residual / lower bound of the published free-chlorine MSC (minimum selective concentration) range 0.021–0.39 mg/L — a precautionary floor, NOT an organism-specific MIC.** *C. necator* has no published MIC, and the genus is chlorine-tolerant (see the per-genus Comments column), so the true impairment concentration is higher. The verdict is, however, **insensitive to the exact threshold**: `HOCl_max` (~8.9 mg/L) exceeds even a 1 mg/L bactericidal level by ~9×, so the RISK flag stands whether the floor is set at the precautionary 0.2 mg/L or a frankly bactericidal 1 mg/L.

**Bench-survival narrative — UNCONFIRMED (not in the repo).** A previously-circulated bench narrative (an "Angella Exp1-lives / Exp2-dies" pH 8→6 survive-vs-kill story) does **not appear anywhere in this repository** and is not used as validation here. It should be treated as unconfirmed. Importantly, **the model does NOT predict alkaline-run survival**: even at pH 8 the HOCl ceiling is ~2.3 mg/L, which still exceeds the 0.2 mg/L impairment gate (and a 1 mg/L bactericidal level). The genuine pH effect the chemistry captures is a **~3.9× SEVERITY contrast** between the acidic CO₂-trough run (HOCl_max ~8.9 mg/L, the H–H fraction near-fully protonated) and an alkaline run (~2.3 mg/L) — i.e. acidic dosing is ~3.9× worse for HOCl, **not** a survive/kill threshold either side of a pH line. Frame the pH dependence as severity, not as a survival boundary.
