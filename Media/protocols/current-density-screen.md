...
- At least 4 electroPioreactor units – one per current density. The unit count is the real barrier here: an 8-point series at 2 replicates each is 16 unit-runs. Run the series in batches if you have fewer units, keeping conditions identical across batches.
...
3. Run the electrolysis arm. Assign one unit (or replicate group) per current density. Blank the onboard OD reader against a cell-free vial of medium only (or bicarbonate-only) before inoculation, then inoculate every unit identically. Log growth continuously and sample for viability at fixed timepoints. Record pH, DO and temperature at each current density throughout – these are the mechanism read-outs. In-culture electrolysis evolves hydrogen and oxygen together; keep the CO₂ sparge running as the inerting purge whenever current flows, do not occlude the vent, ventilate, and exclude ignition.
4. Run the decoupled control arm (the key control). In parallel, grow the strain with H₂/O₂/CO₂ delivered by external sparging, but with no in-culture current (or the lowest current density that still drives the sparger). Match the gas rate to the mid-series electrolytic gas production: read the gas production rate the model reports at the mid-series setpoint and use that as the target sparge rate. This arm shows what growth looks like when substrate is present but electrode stress is absent. The same gas safety applies as for the electrolysis arms – flashback arrestor on the H₂ line, ventilation, ignition excluded.
