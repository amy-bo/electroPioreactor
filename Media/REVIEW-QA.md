...
1. **Free-chlorine model — REBUILT as a graded kinetic penalty (Chemistry 128–147).** Production capped by the chloride mass-transport limit (scales with chloride); steady bulk free HOCl = P/(k₁·NH₃_free), non-zero because only ~0.6% of ammonium is reactive free NH₃ at pH 7 (Jafvert & Valentine 1992); plus combined chlorine ÷25 potency; plus an anode boundary-layer factor; vs a graded onset (0.1)/kill (2) threshold. UdG → C_eff ≈ 0.68 mg/L = MILD sub-lethal penalty (matches Sydow 2017 chloride-free advantage; not sterilising). Uncertain knobs (FE plateau, f_local, monochloramine residence) exposed as flagged sensitivity inputs. Twin 84/84. Remaining residual: the sched/HOCl what-if (D14/D16 + pH-coupling) is still to wire.
